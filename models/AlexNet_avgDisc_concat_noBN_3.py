...
def alexnet_argscope(activation=tf.nn.relu, kernel_size=(3, 3), padding='SAME', training=True, w_reg=0.00001,
                     fix_bn=False):
    ...
    def discriminate(self, net, reuse=None, training=True, with_fc=True):
        ...
        with tf.variable_scope('discriminator', reuse=reuse):
            with slim.arg_scope(alexnet_argscope(activation=self.fc_activation, padding='SAME', training=training,
                ...
                if with_fc:
                    drop_pred = slim.conv2d(net, 1, kernel_size=[3, 3], padding='SAME', activation_fn=None, normalizer_fn=None)
