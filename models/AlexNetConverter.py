...
class AlexNetConverter:
    def __init__(self, model_dir, model, sess, remove_bn=False, ckpt=None, net_id='discriminator', scale=1.0, bgr=False,
                 exclude=None, with_fc=False, im_size=(227, 227), use_classifier=True, num_classes=1000):
        self.model = model
        self.bgr = bgr
        self.sess = sess
        self.net_id = net_id
    ...
    def init_model(self):
        ...
        if self.use_classifier:
            self.model.classifier(x, self.num_classes)
        else:
            self.model.discriminator.discriminate(x, with_fc=self.with_fc, training=False)
    ...
    def get_group_conv(self, layer):
        ...
        weights = tf.concat(axis=3, values=[weights_0, weights_1])
        biases = tf.concat(axis=0, values=[biases_0, biases_1])
        return weights, biases
