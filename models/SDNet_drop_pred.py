...
from AlexNet_drop_pred import AlexNet
...
class SDNet:
    def __init__(self, num_layers, batch_size, target_shape, pool5=True, tag='default', fix_bn=False, disc_pad='VALID'):
        ...
        """
        self.name = 'SDNet_drop_pred_{}'.format(tag)
        self.num_layers = num_layers
        self.batch_size = batch_size
        self.im_shape = target_shape
        self.discriminator = AlexNet(fix_bn=fix_bn, pool5=pool5, pad=disc_pad)
        self.dec_im = self.dec_drop = self.disc_real = self.drop_pred = self.drop_label = self.disc_fake = None
