...
class SDNet:
    def __init__(self, num_layers, batch_size, target_shape, num_res=1, pool5=True, tag='default', fix_bn=False,
        ...
        """
        self.name = 'SDNet_avgDisc_onlyDP_res{}_{}'.format(num_res, tag)
        ...
        self.dec_im = self.dec_drop = self.drop_pred_real = self.drop_pred_fake = self.drop_label = \
    ...
    def net(self, imgs, reuse=None, train=True):
        ...
        __, self.drop_pred_real, __ = self.discriminator.discriminate(self.dec_im, reuse=reuse, training=train)
        __, self.drop_pred_fake, __ = self.discriminator.discriminate(self.dec_drop, reuse=True, training=train)
    ...
    def generator_loss(self, imgs_train):
        ...
        drop_pred_loss = tf.losses.sigmoid_cross_entropy(tf.ones_like(self.drop_label), self.drop_pred_fake, scope=gen_loss_scope,
                                                         weights=1.0)
    ...
    def discriminator_loss(self):
        ...
        drop_pred_loss = tf.losses.sigmoid_cross_entropy(self.drop_label, self.drop_pred_fake, scope=disc_loss_scope,
                                                         weights=1.0)
        tf.summary.scalar('losses/discriminator_drop', drop_pred_loss)
        tf.losses.sigmoid_cross_entropy(tf.ones_like(self.drop_label), self.drop_pred_real, scope=disc_loss_scope,
                                                         weights=1.0)
