...
class SDNet:
    def __init__(self, num_layers, batch_size, target_shape, pool5=True, tag='default', fix_bn=False, disc_pad='VALID'):
    ...
    def net(self, imgs, reuse=None, train=True):
        ...
        enc_shuffle, __ = spatial_shuffle(enc_im, 0.5)
        ...
        self.dec_drop = self.generator(enc_shuffle, reuse=reuse, training=train)

        # Build input for discriminator (discriminator tries to guess order of real/fake)
        self.disc_real, __ = self.discriminator.discriminate(self.dec_im, reuse=reuse, training=train)
        self.disc_fake, __ = self.discriminator.discriminate(self.dec_drop, reuse=True, training=train)
    ...
    def generator_loss(self, imgs_train):
        ...
        fake_loss = tf.losses.sigmoid_cross_entropy(tf.ones_like(self.disc_fake), self.disc_fake,
                                                    scope=gen_loss_scope)
        tf.summary.scalar('losses/generator_fake', fake_loss)
        ae_loss = tf.losses.mean_squared_error(imgs_train, self.dec_drop, scope=gen_loss_scope, weights=30.0)
    ...
    def discriminator_loss(self):
        ...
        real_loss = tf.losses.sigmoid_cross_entropy(tf.ones_like(self.disc_real), self.disc_real,
                                                    scope=disc_loss_scope)
        tf.summary.scalar('losses/discriminator_real', real_loss)
        fake_loss = tf.losses.sigmoid_cross_entropy(tf.zeros_like(self.disc_fake), self.disc_fake,
                                                    scope=disc_loss_scope)
        ...
        real_pred = tf.to_int32(tf.round(tf.nn.sigmoid(self.disc_real)))
        tf.summary.scalar('accuracy/discriminator_real', slim.metrics.accuracy(real_pred, tf.ones_like(self.disc_real,
                                                                                                       dtype=tf.int32)))
        fake_pred = tf.to_int32(tf.round(tf.nn.sigmoid(self.disc_fake)))
        tf.summary.scalar('accuracy/discriminator_fake', slim.metrics.accuracy(fake_pred, tf.ones_like(self.disc_fake,
                                                                                                       dtype=tf.int32)))
