...
from AlexNet_avgDisc_concat_noBN_4 import AlexNet
from layers_new import up_conv2d_new, pixel_dropout_noise, res_block_bottleneck, spatial_drop_noise2
...
class SDNet:
    def __init__(self, num_layers, batch_size, target_shape, num_res=1, pool5=True, tag='default', fix_bn=False,
        ...
        """
        self.name = 'SDNet_avgDisc_concat_new4_res{}_{}'.format(num_res, tag)
    ...
    def net(self, imgs, reuse=None, train=True):
        ...
        pixel_drop, drop_mask = spatial_drop_noise2(enc_im, 0.85, [2, 2])
        ...
        with tf.variable_scope('generator', reuse=reuse):
            with slim.arg_scope(toon_net_argscope(padding='SAME', training=training)):
                ...
                for l in range(0, self.num_layers - 1):
                    net = up_conv2d_new(net, num_outputs=f_dims[self.num_layers - l - 2], scope='deconv_{}'.format(l))
                net = up_conv2d_new(net, num_outputs=32, scope='deconv_{}'.format(self.num_layers))
    ...
    def generator_loss(self, imgs_train):
        ...
        drop_pred_loss = tf.losses.sigmoid_cross_entropy(tf.ones_like(self.drop_label), self.drop_pred,
                                                         scope=gen_loss_scope, weights=1.0)
