...
from AlexNet_avgDisc_concat import AlexNet
...
class SDNet:
    def __init__(self, num_layers, batch_size, target_shape, num_res=1, pool5=True, tag='default', fix_bn=False,
        ...
        """
        self.name = 'SDNet_avgDisc_concat_res{}_{}'.format(num_res, tag)
    ...
    def generator_loss(self, imgs_train):
        ...
        fake_loss = tf.losses.softmax_cross_entropy(self.labels_real(), self.disc_out, scope=gen_loss_scope,
                                                 weights=1.0, label_smoothing=1.0)
        ...
        drop_pred_loss = tf.losses.sigmoid_cross_entropy(tf.ones_like(self.drop_label), self.drop_pred,
                                                         scope=gen_loss_scope, weights=0.2)
    ...
    def discriminator_loss(self):
        ...
        real_loss = tf.losses.softmax_cross_entropy(self.labels_real(), self.disc_out,
                                                 scope=disc_loss_scope, weights=1.0)
