...
class SDNet:
    def __init__(self, num_layers, batch_size, target_shape, num_res=1, pool5=True, tag='default', fix_bn=False,
        ...
        """
        self.name = 'SDNet_drop&pool_res{}_{}'.format(num_res, tag)
        ...
        self.dec_im = self.dec_drop = self.disc_real = self.drop_pred = self.drop_label = self.disc_drop = \
            self.dec_pool = self.rec_weights = self.disc_pool = None
    ...
    def net(self, imgs, reuse=None, train=True):
        ...
        self.dec_drop = self.generator(pixel_drop, drop_mask, reuse=reuse, training=train, tag='drop')
        self.dec_pool = self.generator(enc_pool, reuse=reuse, training=train, tag='pool')
        ...
        self.disc_drop, self.drop_pred, __ = self.discriminator.discriminate(self.dec_drop, reuse=True, training=train)
        self.disc_pool, __, __ = self.discriminator.discriminate(self.dec_pool, reuse=True, training=train)

        tf.summary.image('imgs/dec_pool', montage_tf(self.dec_pool, 2, 8), max_outputs=1)
    ...
    def generator(self, net, drop_mask=None, reuse=None, training=True, tag='default'):
    ...
    def generator_loss(self, imgs_train):
        ...
        drop_loss = tf.losses.softmax_cross_entropy(self.labels_real(), self.disc_drop, scope=gen_loss_scope)
        tf.summary.scalar('losses/generator_drop', drop_loss)
        pool_loss = tf.losses.softmax_cross_entropy(self.labels_real(), self.disc_pool, scope=gen_loss_scope)
        tf.summary.scalar('losses/generator_pool', pool_loss)
        ae_loss = tf.losses.mean_squared_error(imgs_train, self.dec_drop, scope=gen_loss_scope,
                                               weights=30.0*self.rec_weights)
        tf.summary.scalar('losses/generator_mse_drop', ae_loss)
        ae_loss = tf.losses.mean_squared_error(imgs_train, self.dec_pool, scope=gen_loss_scope,
                                               weights=30.0)
        tf.summary.scalar('losses/generator_mse_pool', ae_loss)
    ...
    def discriminator_loss(self):
        ...
        drop_loss = tf.losses.softmax_cross_entropy(self.labels_fake(), self.disc_drop, scope=disc_loss_scope)
        tf.summary.scalar('losses/discriminator_drop', drop_loss)
        pool_loss = tf.losses.softmax_cross_entropy(self.labels_fake(), self.disc_pool, scope=disc_loss_scope)
        tf.summary.scalar('losses/discriminator_pool', pool_loss)
        ...
        drop_pred = tf.arg_max(self.disc_drop, 1)
        tf.summary.scalar('accuracy/discriminator_drop', slim.metrics.accuracy(drop_pred, tf.zeros_like(drop_pred)))
        pool_pred = tf.arg_max(self.disc_pool, 1)
        tf.summary.scalar('accuracy/discriminator_pool', slim.metrics.accuracy(pool_pred, tf.zeros_like(pool_pred)))
